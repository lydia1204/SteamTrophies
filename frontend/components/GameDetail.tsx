    ...
    <div ref={scrollRoot} onScroll={remember} className="st-detail" data-stt-component="game-detail" data-stt-appid={game.appId}>
      <div className="st-detail-header" data-stt-slot="header">
        ...
        <div className="st-detail-actions">
          ...
          <button className={`st-icon-button${pinned ? ' is-pinned' : ''}`} title={message(pinned ? 'app.unpin' : 'app.pin')} aria-label={message(pinned ? 'app.unpin' : 'app.pin')} aria-pressed={pinned} onClick={() => void customizationService.setPinned(game.appId, !pinned)}><SheetIcon control="pin" tint={pinned} />{pinned && <span className="st-pin-remove" aria-hidden="true">−</span>}</button>
