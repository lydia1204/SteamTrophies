...
function AchievementTooltip({ anchor, achievement: a, appId, platinum, platinumDate }: { anchor: HTMLButtonElement; achievement: AchievementRecord | null; appId: number; platinum?: boolean; platinumDate?: number | null }) {
  ...
  }, [anchor, a, view, customization.config.accessibility, customization.config.visual.tooltipLayout]);
  const tokens = customizationService.getCssVariables('desktop');
  return createPortal(<div ref={element} className={`st-achievement-tooltip${tier === 'gold' || platinum || customization.config.visual.allTierEffects ? ` st-aura st-aura-${tier}` : ''}`} data-layout={customization.config.visual.tooltipLayout} data-stt-reduced-motion={customization.config.accessibility.reducedMotion} role="tooltip" style={{ ...position, ...tokens } as CSSProperties}>
