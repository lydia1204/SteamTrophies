...
export function LayoutEditor({ surface }: { surface: SurfaceKind }) {
  ...
  return (
    <section className="stt-settings-section" data-stt-component="layout-editor" data-stt-surface={surface}>
      <header><div><h3>Dashboard layout</h3><p>Reorder or hide shelves for this surface only.</p></div><button title="Reset" className="st-mini-button" onClick={() => void customizationService.resetLayout(surface)}>Reset</button></header>
          ...
          return (
            <div className="stt-layout-row" key={id} data-stt-widget={id}>
              ...
              <div className="stt-layout-actions">
                <button title={`Move ${definition.title} up`} className="st-mini-button" disabled={index === 0} onClick={() => void customizationService.moveWidget(surface, id, -1)} aria-label={`Move ${definition.title} up`}>↑</button>
                <button title={`Move ${definition.title} down`} className="st-mini-button" disabled={index === layout.order.length - 1} onClick={() => void customizationService.moveWidget(surface, id, 1)} aria-label={`Move ${definition.title} down`}>↓</button>
                <button title={hidden ? 'Show' : 'Hide'} className="st-mini-button" onClick={() => void customizationService.setWidgetVisible(surface, id, hidden)}>{hidden ? 'Show' : 'Hide'}</button>
              </div>
            </div>
          );
        })}
